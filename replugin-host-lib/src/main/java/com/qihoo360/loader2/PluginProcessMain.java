...
//            }
    ...
    /**
     * 首先PluginProviderStub.proxyFetchHostBinder(context)获取了一个IBinder对象，接着下面通过调用asInterface方法确定是否需要返回远程代理，
     * 然后调用PluginManagerProxy.connectToServer(sPluginHostRemote)方法，这个方法在第9步中，PluginProcessMain的installHost也调用过这个方法，
     * 它里面调用了IPluginHost.fetchManagerServer()方法将PluginManagerServer中的Stub对象，
     * 也就是IPluginServiceServer类型的Binder对象赋值给PluginManagerProxy类中的字段sRemote，
     * 这个IPluginServiceServer类型的Binder对象掌握了对插件的安装、卸载、更新等等的操作。
     * 最后调用了PMF.sPluginMgr.attach()方法将当前进程信息注册到“插件管理进程”中来统一管理。
     * <p>
     * <p>
