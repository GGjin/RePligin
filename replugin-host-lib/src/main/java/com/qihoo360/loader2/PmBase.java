...
class PmBase {
     ...
     * 插件管理进程最主要干的事情就是创建了PmHostSvc这个Binder对象，它其中还管理着另外两个重要的Binder对象一个是用来管理Service的PluginServiceServer，
     * 还有一个是来管理插件安装、卸载等的PluginManagerServer。
     * 接着从assest的json文件中和v5安装目录中扫描本地的插件信息，将信息添加进一个叫mPlugins的ConcurrentHashMap中，
     * 然后再去查看之前是否有已经安装的插件，如果有判断是否需要更新、删除等操作，并做相应的处理后返回处理后的插件信息集合，
     * 将集合中的内容也添加进mPlugins中。存入的key是包名或者别名，value是Plugin
     * <p>
     * 其他进程：initForClient()
     * 这个进程干的事情就简单了，就是通过Provider的方式请求插件管理进程返回PmHostSvc这个Binder对象，
     * 接着通过PmHostSvc再得到PluginManagerServer这个Binder对象并把当前进程信息注册到Server端，最后通过得到的Binder对象来同步进程信息和更新插件信息，
     * <p>
     * 2）在上面各进程操作完后会遍历mPlugins中的所有插件信息，并将他们存入到一个叫PluginTable类中一个叫PLUGINS的HashMap中，
     * 存入的key是包名或者别名，value是PluginInfo，这就是
     */
    void init() {
     ...
     * initForServer ->
     * <p>
     * 1、首先创建了一个PmHostSvc对象，这个类继承IPluginHost.Stub，是一个IPluginHost类型的Binder对象，
     * 可以说所有的插件的管理工作都是直接或者间接由它处理的，PmHostSvc它代表了Server端要处理的事情，也就是插件管理进程处理的事情
     * <p>
     * <p>
     * 2、在PmHostSvc的构造方法中又创建了两个对象，一个是PluginServiceServer，这个类是用来管理插件Service的远程Server端，
     * 还有一个是PluginManagerServer，这个类在创建的时候在构造中又创建了一个继承自IPluginServiceServer.Stub的Stub对象，
     * Stub也是一个Binder对象，通过后来查看IPluginServiceServer的代码，发现这个类掌管了所有对插件的的操作，例如插件的安装、加载、卸载、更新等等
     * <p>
     * <p>
     * 3、调用PluginProcessMain.installHost(mHostSvc)方法将PmHostSvc对象也就是IPluginHost类型赋值给PluginProcessMain中的字段sPluginHostLocal，
     * 这个IPluginHost是Binder对象。接着调用了IPluginHost.fetchManagerServer()方法将PluginManagerServer中的Stub对象，
     * 也就是IPluginServiceServer类型的Binder对象赋值给PluginManagerProxy类中的字段sRemote，
     * 这个IPluginServiceServer类型的Binder对象掌握了对插件的安装、卸载、更新等等的操作
     * <p>
     * <p>
     * 4、剩下的几行代码全部都是来更新插件信息的。先搜索本地插件和V5插件信息，创建插件模型并添加进Builder.PxAll相应的集合中，
     * 然后判断删除一些不符合规则的信息，然后同步所有的插件信息，最后判断是否有插件需要更新或删除，对应的执行一些操作。
     * 本地插件的扫描是通过assest目录下的一个叫plugins-builtin.json的文件，这个文件是replugin-host-gradle插件自动生成的。
     * V5插件是通过context.getDir的文件目录遍历
     */
    ...
    /**
     * Client(UI进程)的初始化
     * 如果使用了常驻进程，ui进程对于常驻进程来说也是Client
     */
